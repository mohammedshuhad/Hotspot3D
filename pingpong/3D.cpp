...
void hotspot3D(int flag, float pIn[TILE_LAYERS * GRID_ROWS * GRID_COLS], float tIn[(TILE_LAYERS + 2) * (GRID_ROWS)*GRID_COLS], float tOut[TILE_LAYERS * GRID_ROWS * GRID_COLS], int l)
{
    if(flag)
    {
    int  w, e, n, s, b, t;
    ...
    ITER_LOOP:for ( i = 0; i < ITERATIONS ; i++)
        ...
        TILE_LOOP:for (l = 0; l < GRID_LAYERS + 2; l+= TILE_LAYERS)
        {
            ...
            if(l % 2 == 0) 
            {
              ...
              hotspot3D(compute_flag, power_inner_1, temp_inner_1, result_inner_1, l - 1);
              store(store_flag, tempOut, result_inner_0, l - 2);
            }
            else if (l % 2 == 1) 
            {
              load(load_flag, temp_inner_1,power_inner_1, tempIn, powerIn, l );
              hotspot3D(compute_flag, power_inner_0, temp_inner_0, result_inner_0, l - 1);
